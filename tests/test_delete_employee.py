...
def test_delete_employee(logged_in_user,data):
   ...
   pim_page.delete_employee(emp_id)
   expect(logged_in_user.locator(".oxd-text.oxd-text--p.oxd-text--card-title")).to_have_text("Are you Sure?")
   pim_page.confirm_delete()
   pim_page.enter_employee_name(f"{data['firstname']} {data['lastname']}",emp_id)
   expect(pim_page.get_no_records_found_text()).to_be_visible()